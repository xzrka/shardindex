...
impl CppParser {
    pub fn new() -> Result<Self, anyhow::Error> {
        ...
        parser
            ...
            .context("Failed to load tree-sitter-cpp")?;
        Ok(Self)
    }
    ...
    fn walk_node(node: &Node, source: &[u8], result: &mut ParseResult, parent: Option<String>, current_function: Option<String>) {
        ...
        match kind {
            ...
            "namespace_definition" => {
                Self::extract_namespace_def(node, source, result, parent.as_deref());
            }
            _ => {}
        }
    ...
    fn extract_class(node: &Node, source: &[u8], result: &mut ParseResult, parent: Option<&str>) {
        ...
        let base = node
            .child_by_field_name("base_class")
            .and_then(|n| n.utf8_text(source).ok())
            .map(|s| s.to_string());

        let sig = if let Some(b) = &base {
            format!("class {} : {}", name, b)
        } else {
            format!("class {}", name)
        };
    ...
    fn extract_namespace(node: &Node, source: &[u8], result: &mut ParseResult, parent: Option<&str>) {
        let name = node
            .child_by_field_name("name")
            .and_then(|n| n.utf8_text(source).ok())
            .map(|s| s.to_string());
        if let Some(name) = name {
            result.symbols.push(ParsedSymbol {
                ...
                start_col: node.start_position().column,
                end_col: node.end_position().column,
                signature: Some("namespace".to_string()),
                docstring: None,
                parent: parent.map(|s| s.to_string()),
            });
        }
    ...
    fn extract_namespace_def(node: &Node, source: &[u8], result: &mut ParseResult, parent: Option<&str>) {
        let name = node
            .child_by_field_name("name")
            .and_then(|n| n.utf8_text(source).ok())
            .map(|s| s.to_string());
...
impl SourceCodeParser for CppParser {
