...
impl TypeScriptParser {
        ...
        for child in node.children(&mut cursor) {
            ...
            let new_function = match child.kind() {
                "function_declaration" => child
                    ...
                    .and_then(|n| n.utf8_text(source).ok())
                    .map(|s| s.to_string()),
                "method_definition" | "abstract_method_definition" => child
    ...
    fn extract_import(node: &Node, source: &[u8], result: &mut ParseResult) {
        ...
        for name in &names {
            result
                .imports
                .push((cleaned.clone(), name.clone(), "import".to_string()));
            result.references.push(ParsedReference {
                caller_symbol: None,
    ...
    fn extract_export(node: &Node, source: &[u8], result: &mut ParseResult, _parent: Option<&str>) {
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
